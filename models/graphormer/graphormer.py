...
class GraphormerEncoder(nn.Module):
    ...
    def forward(self, batched_data, perturb=None, masked_tokens=None, return_node_emb=False, **unused):
        if 'org_x' in batched_data: # both task: mask and identi
            inner_states, graph_rep = self.graph_encoder(
                batched_data,
                perturb=perturb,
            )
            mask_x = inner_states[-1].transpose(0, 1) # mask feature
            
            batched_data['x'] = batched_data['org_x']
            batched_data['attn_edge_type'] = batched_data['org_attn_edge_type']
            batched_data['edge_input'] = batched_data['org_edge_input']
            
            org_inner_states, org_graph_rep = self.graph_encoder(
                batched_data,
                perturb=perturb,
            )
            x = org_inner_states[-1].transpose(0, 1) # org for identi
        else:
            inner_states, graph_rep = self.graph_encoder(
                batched_data,
                perturb=perturb,
        ...
        if not self.finetune:
            if 'org_x' in batched_data:
                mask_x = self.get_reaction_node_embedding(mask_x)
        ...
        if self.mlabel_task:
            if 'org_x' in batched_data:
                mlabel_logits = self.mlabel_head(mask_x)
            else:
